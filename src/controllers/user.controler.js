...
const updateUserAvatar = asyncHandler(async (req, res) => {
  try {
    const avataLocalPath = req.file?.avatar.path;
    ...
    throw new ApiError(400, error?.messgae || "avatar not update successful");
  }
});

const updateCoverImage = asyncHandler(async (req, res) => {
  const coverImagePath = await req.file?.coverImage?.path;
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { user }, "coverImage file successfully updated")
    );
});
